...
function llh_test = findIntercept_LEO(h_intercept,u_sat,rng_sat,rx_traj_llh)
...
% cosines: (R + h_sat)^2 = Rx^2 + rng_sat^2 - 2*Rx^2*rng_sat^2*cos(gamma)
cos_gamma = -((R + h_sat).^2-rng_sat.^2  - Rx.^2)./(2*Rx.*rng_sat);

% Use law of cosines again to find intercept range, r_test
% (R + h_intercept)^2 = Rx^2 + r_test^2 - 2*Rx*r_test*cos(gamma)
% r_test^2 - 2*R*cos(gamma)*r_test - (2*R*h_intercept + h_intercept^2) = 0
nsamps=length(cos_gamma);
r_test=zeros(size(cos_gamma));
for nsamp=1:nsamps
  r_test(nsamp) = max(roots([1, -2*Rx(nsamps)*cos_gamma(nsamp), Rx(nsamps)^2-(R + h_intercept)^2]));  
end
